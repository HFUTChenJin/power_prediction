...
public interface PowerDistributionDayRepository extends JpaRepository<PowerDistributionDay,Integer> {
    ...
    @Query(value = "select device.name,power_distribution_day.* " +
            "from device,power_distribution_day " +
            "where power_distribution_day.deviceId in (:idList) " +
            "and device.id=power_distribution_day.deviceId " +
            "and power_distribution_day.dataTime=:dataTime", nativeQuery = true)
    ...
    @Query(value = "select device.name,power_distribution_day.* " +
            "from device,power_distribution_day " +
            "where power_distribution_day.deviceId=:deviceId " +
            "and device.id=power_distribution_day.deviceId " +
            "and power_distribution_day.dataTime between :start and :timeEnd " +
            "order by power_distribution_day.dataTime ASC ", nativeQuery = true)
