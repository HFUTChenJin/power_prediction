...
public interface PowerDistributionHourRepository extends JpaRepository<PowerDistributionHour,Integer> {
    ...
    @Query(value = "select device.name,sum(power_distribution_hour.totalKWh) " +
            "from device,power_distribution_hour" +
            " where power_distribution_hour.deviceId=:deviceId and device.id=power_distribution_hour.deviceId" +
            " and power_distribution_hour.dataTime between :start and :timeEnd " +
            "order by power_distribution_hour.dataTime ASC ", nativeQuery = true)
